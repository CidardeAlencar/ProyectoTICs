<?php 
	...
	$per_page = 20;
		...
		$count = $db->query("SELECT * FROM productos");
		...
		$cat = $db->query("SELECT * FROM productos LIMIT $start, $per_page");
		...
		if($cat->num_rows) {
			...
			while($row = $cat->fetch_assoc()) {
				...
				$id = $row['id_producto'];
				$foodName = $row['nombre'];
				$foodCat = $row['categoria'];
				$foodPrice = $row['precio'];
				$foodDesc = $row['descripcion'];
        ...
        <nav class="navbar navbar-default navbar-fixed" style="background: #FF5722;">
            <div class="container-fluid">
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand" href="#" style="color: #fff;">COLECCIÓN DE PRODUCTOS</a>
                </div>
