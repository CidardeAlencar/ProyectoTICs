...
<head>
    ...
    <style>
        ...
        /* Estilos para el label que actúa como el toggle switch */
        .switch {
            position: relative;
            display: inline-block;
            width: 60px;
        ...
        <div class="table-container">
            ...
            <table>
                ...
                <tbody>
                    ...
                    $sql = "SELECT id, nombreTrabajador, cargoDesempeno, ocasional, mitad, frecuente, siempre, estado FROM evaluaciondesempeno";
                    ...
                    if (isset($_GET['buscar']) && !empty($_GET['buscar'])) {
                        ...
                        $sql_busqueda = "SELECT id, nombreTrabajador, cargoDesempeno, ocasional, mitad, frecuente, siempre, estado 
                    ...
                    foreach ($resultados as $row) {
                        // Determinar el texto y la clase CSS basado en el estado
                        $estadoTexto = $row["estado"] == 1 ? 'On' : 'Off';
                        $estadoClase = $row["estado"] == 1 ? 'on-btn' : 'off-btn';
                        ...
                        <div class='toggleContainer'>
                            <input type='checkbox' id='switch-" . htmlspecialchars($row["id"]) . "' class='toggle' " . ($row["estado"] == 1 ? 'checked' : '') . ">
                            <label for='switch-" . htmlspecialchars($row["id"]) . "' class='switch'></label>
                        </div>
                        </td>";
                        echo "</tr>";
                    }
...
?>
<script>
    // Función para manejar el cambio de estado del toggle switch
    function cambiarEstado(id, nuevoEstado) {
        // Hacer una solicitud AJAX para actualizar el estado en la base de datos
        ...
        xhttp.open("GET", "cambiar_estadoCATE.php?id=" + id + "&estado=" + nuevoEstado, true);
        xhttp.send();
    }

    // Escuchar el evento change en los toggle switches
    document.addEventListener("DOMContentLoaded", function() {
        let switches = document.querySelectorAll('.toggle');
        switches.forEach(function(switchElem) {
            switchElem.addEventListener('change', function() {
                let id = this.id.split('-')[1]; // Obtener el ID del switch
                let nuevoEstado = this.checked ? 1 : 0; // Determinar el nuevo estado
                cambiarEstado(id, nuevoEstado); // Llamar a la función para cambiar el estado
            });
        });
    });
</script>