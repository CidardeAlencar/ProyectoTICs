...
if ( $id !== '' ) {
  $currentDiscountStmt = $connect->prepare( "
    SELECT 
    *
    FROM
    descuentos
  ...
  $currentDiscountStmt->bind_param( 'i', $id );
  $currentDiscountStmt->execute();
  ...
  $newStatus = $currentDiscount[ 'status' ] === 1 ? 0 : 1;


  $updateDiscountStmt = $connect->prepare( "
    UPDATE descuentos
    SET status = ?
    WHERE id = ?
    " );
